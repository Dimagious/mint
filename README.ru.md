...
<p align="center">
  <!-- TODO: hero.gif — 8-12 с цикл: бросаем фото → пишем заголовок → Smart Contrast → экспорт -->
...
<!-- TODO: workflow.gif — 12 с: бросаем фото, пишем заголовок, жмём Smart Contrast, экспорт PNG -->
<img src="docs/demo/workflow.gif" alt="Фото сбрасывается на холст. Пишется заголовок. Smart Contrast делает текст белым с мягкой обводкой. Открывается диалог экспорта, PNG скачивается." width="900" />

### 2. Snap по краям слоёв

<!-- TODO: snap.gif — 8 с: тащим слой рядом с другим, появляется мятная направляющая, дотягиваем до канваса -->
...
<!-- TODO: palette.gif — 6 с: жмём Cmd+K, набираем «exp», Enter → открывается диалог экспорта; снова Cmd+K, набираем «story» → переключается пресет -->
<img src="docs/demo/palette.gif" alt="Cmd+K открывает командную палитру. Пользователь печатает 'export', выбрана команда экспорта, открывается диалог. Палитра открывается снова, набирается 'story', холст переключается в пресет 1080x1920." width="900" />

### 4. Поделиться через URL

<!-- TODO: share.gif — 10 с: композиция готова, More → Share link → ссылка в буфере. Новая вкладка → вставка → тот же дизайн открывается -->
<img src="docs/demo/share.gif" alt="Готовая композиция. Пользователь нажимает More → Share link, тост сообщает 'Ссылка скопирована'. Открывается новая вкладка, вставляется URL, загружается тот же дизайн (без фонового фото, с подсказкой добавить своё)." width="900" />

### 5. Переместить фоновое фото

<!-- TODO: bg-drag.gif — 8 с: фото на холсте, тащим внутри рамки; угловой хэндл — зум; кнопка Reset position -->
<img src="docs/demo/bg-drag.gif" alt="Фото на холсте перетаскивают внутри рамки, потом увеличивают угловым хэндлом, потом кнопка 'Сбросить положение' возвращает фото к auto-fit." width="900" />

### 6. Мобильный

<!-- TODO: mobile.gif — 12 с на мобильном вьюпорте: empty-state, добавляем текст, открываем drawer Style свайпом, меняем цвет, экспорт -->
<img src="docs/demo/mobile.gif" alt="MINT на экране телефона. Тап Add text на empty-state, свайп открывает drawer Style, меняется цвет, тап Export." width="400" />
